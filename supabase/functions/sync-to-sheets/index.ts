...
import { entryToRow, journalFromOrder, journalFromTransaction } from './journal.ts'
...
async function backfill(token: string) {
  ...
  for (const t of txs) {
    // Skip auto-logged transactions that duplicate supplier/affiliate payment entries
    // (they are identified by the [auto-logged] suffix added by the admin panel)
